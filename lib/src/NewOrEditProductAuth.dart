...
class NewOrEditProductAuth extends StatefulWidget {
  Report currentReport;

  Record recordforEdit;

  NewOrEditProductAuth({this.currentReport,this.recordforEdit});

  @override
  NewOrEditProductAuthState createState() {
    return NewOrEditProductAuthState(currentReport,recordforEdit);
  }
}

class NewOrEditProductAuthState extends State<NewOrEditProductAuth> {
  Report currentReport;
  ...
  NewOrEditProductAuthState(this.currentReport,this.recordForEdit);
  ...
  void initState() {
    ...
    //txtCodeControlller.addListener(inputCodeOnChange);
  }
            ...
            onPressed: () {
              ...
              if (_formKey.currentState.validate() && _selectDateValidated && motivoDevolucion != 'Motivo De Devolucion' ) {
                ...
                if(recordForEdit != null){
                  ...
                  if (currentReport.recordsForThisAuth == null){
                    currentReport.recordsForThisAuth = List<Record>();
                    print('no habia ningun record creado!');
                  }
                  currentReport.recordsForThisAuth.add(newRecord);
                }
  ...
  Future barcodeScanning() async {
    ...
    try{
      switch(HotData.currentLab){
        case 'LEGRAND':
          print('es legraaaand' + 'LPT'+barcode.substring(7,barcode.length-1));
    ...
    try{
      switch(HotData.currentLab){
        case 'LEGRAND':
          ...
          print('es legraaaand' + txtDescription.text);
